...
class MultiDataset(Dataset):
    def __init__(
        ...
        **dataset_kwargs
        ...
        assert len(
            self.dataloaders) > 1, "multiple source/target filepairs required for MultiDataset"
        ...
        self.effective_length = int(
            np.sum(datapoint_counts * self.sampling_probs))
    ...
    def __getitem__(self, index):
        if self.current_loader_count == 0:
            self.current_dataset_idx = np.random.choice(
                range(len(self.dataloaders)), p=self.sampling_probs)
        ...
        self.current_loader_count = (
            self.current_loader_count + 1) % self.total_batch_size
        ...
        if self.current_loader_count == self.per_gpu_effective_batch_size:
            ...
            self.shift_iterator(self.current_dataset_idx, self.total_batch_size -
                                self.current_loader_count - self.pos_shift_count)
            self.current_loader_count = 0
...
class AbstractSeq2SeqDataset(Dataset):
    def __init__(
        ...
        **dataset_kwargs
        ...
        file_src = open(self.src_file, 'r')
        file_tgt = open(self.tgt_file, 'r')
        self.data_source = [json.loads(line) for line in file_src]
        self.data_target = [json.loads(line) for line in file_tgt]
        self.data_source = [
            line for line in self.data_source if len(line) > 0]
        self.data_target = [
            line for line in self.data_target if len(line) > 0]

        if n_obs is not None and n_obs != -1:
            mini_indices = random.sample(
                list(range(len(self.data_source))), min(n_obs, len(self.data_source)))
            self.data_source = [self.data_source[index]
        ...
        if os.path.exists(self.len_file):
            ...
            self.src_lens = self.get_char_lens(self.data_source)
    ...
    def tgt_lens(self):
        ...
        return self.get_char_lens(self.data_target)
...
class Seq2SeqDataset(AbstractSeq2SeqDataset):
    ...
    def __getitem__(self, index) -> Dict[str, str]:
        ...
        #tgt_line = linecache.getline(str(self.tgt_file), index).rstrip("\n")
        source_line = self.prefix + self.data_source[index]
        tgt_line = self.data_target[index]
        assert source_line, f"empty source line for index {index}"
        assert tgt_line, f"empty tgt line for index {index}"
...
class Seq2SeqDataCollator:
    def __init__(self, tokenizer, data_args, padding=None, tpu_num_cores=None):
        ...
        self.padding = padding if padding is not None else (
